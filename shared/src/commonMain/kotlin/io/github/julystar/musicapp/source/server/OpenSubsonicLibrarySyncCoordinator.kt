...
typealias OpenSubsonicLibrarySyncResult = RemoteServerLibrarySyncResult

class OpenSubsonicLibrarySyncCoordinator(
    ...
    private val delegate = RemoteServerLibrarySyncEngine(
        ...
        trackDao = trackDao,
        metadataDao = metadataDao,
        providerKind = RemoteServerKind.OpenSubsonic,
    )

    suspend fun sync(
        accountId: SourceAccountId,
        scanId: String = "open-subsonic-${currentTimeMillis()}",
        pageSize: Int = 500,
    ): OpenSubsonicLibrarySyncResult = delegate.sync(accountId, scanId, pageSize)
}
