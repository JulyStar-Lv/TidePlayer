...
object NeteaseYrcParser : ILyricsParser {
    ...
    override fun canParse(content: String): Boolean {
        return content.lineSequence().any { line ->
            val match = lineRegex.matchEntire(line.trim())
            match != null && syllableRegex.containsMatchIn(match.groupValues[3])
        }
    }
    ...
    private fun parseLine(rawLine: String): ISyncedLine? {
        ...
        val rawSyllables = syllableRegex.findAll(content).mapNotNull { syllableMatch ->
            val start = syllableMatch.groupValues[1].toIntOrNull() ?: return@mapNotNull null
            val duration = syllableMatch.groupValues[2].toIntOrNull() ?: return@mapNotNull null
            Triple(start, duration, syllableMatch.groupValues[3])
        }.toList()
        ...
        val syllables = rawSyllables.map { (rawStart, duration, text) ->
            val start = if (usesRelativeTiming) lineStart + rawStart else rawStart
            KaraokeSyllable(
                content = text,
                start = start,
                end = start + duration,
