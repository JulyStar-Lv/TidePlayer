...
fun RecentlyPlayedRoot(
    ...
    LaunchedEffect(viewModel) {
        viewModel.events.collect { event ->
            when (event) {
                is RecentlyPlayedEvent.ShowMessage -> toastRepository.emit(event.message)
            }
        }
    }
        ...
        onAction = { action ->
            when (action) {
                ...
                RecentlyPlayedAction.PlayAll -> {
                    val items = state.tracks.map(RecentlyPlayedTrackItem::toPlayableItem)
...
private fun RecentlyPlayedTrackItem.toPlayableItem() = PlayableItem(
    mediaId = mediaId,
    title = title,
    artist = artist,
